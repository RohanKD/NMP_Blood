...
if __name__ == '__main__':
    ...
    # loads in dataset
    adata = anndata.read_h5ad("../data/scnet_mouse/Mouse_pFC.h5ad")  # test
    bdata = anndata.read_h5ad("../data/scnet_mouse/Mouse_wholebrain_FC.h5ad")  # train
    cdata = anndata.read_h5ad("../data/scnet_mouse/Mousecortex_protocols.h5ad")  # train
    ...
    for sample in samples:
        if counter == 0:
            ...
            train_adata = anndata.AnnData.concatenate(*[train_adata, adata[adata.obs['Sample'] == sample]],
                                                      join="inner")
        elif (counter == train_num):
    ...
    for sample in b_samples:
        if counter == 0:
            ...
            b_test_adata = anndata.AnnData.concatenate(*[b_test_adata, bdata[bdata.obs['sampleID'] == sample]],
                                                       join="inner")
    ...
    combined_test_adata = combined_test_adata[:,
                          list(
                              combined_train_adata.var_names)]  ## extract out the features selected in the training dataset
    test_data_mat = _utils._extract_adata(combined_test_adata)
    test_data_mat = (test_data_mat - np.array(combined_train_adata.var['mean'])) / np.array(
        combined_train_adata.var['std'])
    ...
    combined_train_adata.var.loc[:, ['mean', 'std']].to_csv(
